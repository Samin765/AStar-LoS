...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAI : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            start_pos.x = (float)Math.Round(start_pos.x);
            start_pos.z = (float)Math.Round(start_pos.z);
            goal_pos.x = (float)Math.Round(goal_pos.x);
            goal_pos.z = (float)Math.Round(goal_pos.z);
            //whther the start and the goal are in the map or the blocked area
            if (!mapData.ContainsKey(new Vector2Int((int)start_pos.x, (int)start_pos.z)) || mapData[new Vector2Int((int)start_pos.x, (int)start_pos.z)] == ObstacleMap.Traversability.Blocked)
            {
                Debug.Log("the start is not in the map or the blocked area");
                return;
            }
